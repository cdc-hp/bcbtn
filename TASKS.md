...
### Bổ sung lần 3 — lỗi thứ 4: bước chẩn đoán ở trên phát huy tác dụng ngay lần đầu

Bản chẩn đoán thêm vào CI ở trên (Event Log + chạy thử trực tiếp) phát huy tác dụng NGAY LẦN
ĐẦU tiên chạy lại — tránh phải đoán mù thêm 1 vòng như 2 lần trước. Log Event thật:

```
The instance's SvcRun() method failed
Traceback (most recent call last):
 File "win32serviceutil.py", line 1062, in SvcRun
 File "service_windows.py", line 137, in SvcDoRun
pywintypes.error: (5, 'RegisterEventSource/ReportEvent', 'Access is denied.')
```

Xác nhận sửa lỗi thứ 3 (host mode) ĐÚNG — dịch vụ giờ đã vào tới `SvcDoRun()` thật sự (trước đó
còn chưa tới được bước này). Lỗi mới: `SvcDoRun()` gọi `servicemanager.LogMsg(...)` (chỉ để ghi
1 dòng "đã khởi động" vào Windows Event Log) — hàm này cần nguồn sự kiện (event source) đã đăng
ký sẵn trong registry trỏ tới 1 DLL thông báo (message DLL); việc đăng ký này bình thường do
`pythonservice.exe` (mô hình dịch vụ Python KHÔNG đóng gói) lo, nhưng dịch vụ này tự host chính
nó (đúng thiết kế từ đầu — không dùng `pythonservice.exe`, xem 2 mục trên) nên không có đăng ký
đó, khiến `LogMsg` ném lỗi và làm cả `SvcDoRun()` thất bại NGAY DÒNG ĐẦU — `run_server()` chưa
từng được gọi tới. Sửa: bọc `try/except` quanh lệnh gọi `LogMsg` (chỉ mang tính thông tin, không
ảnh hưởng việc phục vụ HTTP) — lỗi ghi log bị bỏ qua an toàn, `run_server(self)` vẫn chạy tiếp.
Thêm `tests/test_service_windows.py::test_svc_do_run_survives_event_log_failure` hồi quy.

**Bài học lớn nhất từ cả 3 lần bổ sung này**: freeze 1 dịch vụ pywin32 bằng PyInstaller (tự host
chính nó, không qua `pythonservice.exe`) có NHIỀU điểm khác biệt tinh vi so với dùng pywin32 từ
mã nguồn thường — không có cách nào xác minh đầy đủ những điểm này ngoài cài đặt dịch vụ thật và
để Windows SCM thật sự khởi động nó. Bước CI "Verify Web App installs and runs as a Windows
Service" (Giai đoạn 10) — ĐẶC BIỆT là phần chẩn đoán bổ sung (Event Log + chạy `run` trực tiếp)
...
### ĐÃ PHÁT HÀNH THÀNH CÔNG — v0.11.0 (2026-07-24)

Sau 6 lỗi thật liên tiếp (4 lỗi ứng dụng: preprocessor Inno Setup, đường dẫn tương đối
templates/static, dùng sai API pywin32 để tự host dịch vụ, `servicemanager.LogMsg`/Event Log
không có quyền, `uvicorn.Config` tự dựng formatter console không có console; 2 lỗi ở script CI:
dò `/health` quá sớm/không đủ chẩn đoán), lần chạy CI thứ 8 **thành công toàn bộ**, kể cả bước
"Verify Web App installs and runs as a Windows Service" (cài đặt thật, dịch vụ vào đúng trạng
thái Running, `/health` trả lời đúng, gỡ cài đặt sạch qua uninstaller thật do Inno Setup sinh
ra). GitHub Release `v0.11.0` đã tạo, công khai tại
`https://github.com/cdc-hp/bcbtn/releases/tag/v0.11.0`, đủ 5 file:
`CDC-GiamSatDichBenh-Server-Setup-v0.11.0.exe` (Web App tập trung, mới),
`GiamSatDichBenh-Setup-v0.11.0.exe`, `GiamSatDichBenh-Server-Setup-v0.11.0.exe`,
`GiamSatDichBenh-Admin-Setup-v0.11.0.exe` (3 bộ cài desktop cũ), `SHA256SUMS.txt`.

Đây là xác nhận **khách quan, tự động, trên máy Windows thật có quyền Administrator** rằng Web
App tập trung cài đặt và chạy được như dịch vụ Windows đúng như thiết kế — điều kiện còn thiếu
duy nhất trước đây để cân nhắc dọn dẹp `app.py`/`lan_server.py`/`setup*.iss` cũ (xem đầu mục
"Đang làm" phía trên) giờ đã có 1 phần bằng chứng mạnh, nhưng **vẫn nên có người CDC thật sự cài
