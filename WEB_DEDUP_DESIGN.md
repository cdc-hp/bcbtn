...
**Lưu ý thực tế triển khai**: theo mặc định máy chủ chính chỉ nghe trong LAN nội bộ CDC (mục
"Lưu ý mạng LAN" trong `README.md`), nên các xã không cùng mạng LAN **không** vào thẳng được
mục 7.1. Với phần lớn xã ở xa, mục 7.2 (Google Apps Script) mới là link cố định họ dùng hằng
tuần. **Đã cài đặt (mục 7.2)**: nếu CDC chọn mở máy chủ chính ra Internet (domain/IP công khai
+ port forward, xem `google_apps_script/README.md`), Apps Script sẽ **chuyển tiếp trực tiếp**
mỗi lần nộp sang `/queue/submit` của máy chủ chính theo thời gian thực — dữ liệu vào thẳng hàng
đợi chính, không có độ trễ đồng bộ. Chỉ khi máy chủ chính không phản hồi được (mất mạng, đang
bảo trì, hoặc CDC chưa muốn mở ra Internet) thì Apps Script mới đệm tạm vào Sheet/Drive như
thiết kế gốc bên dưới. CSDL chính luôn là SQLite trên máy chủ chính trong mọi trường hợp — Apps
Script không bao giờ là nơi lưu trữ chính thức, kể cả khi chuyển tiếp thất bại và phải đệm tạm.
...
### 7.2 Google Apps Script — link cố định cho các xã ở xa, chuyển tiếp trực tiếp khi có thể

- Google Apps Script publish dưới dạng **Web App**, xử lý cả `doGet` (trang nộp) và `doPost`
  (nhận file base64 từ form).
- **Chuyển tiếp trực tiếp trước** (`Code.gs: tryForwardToMainServer`): nếu Script Property
  `MAIN_SERVER_URL` được cấu hình, mỗi lần nộp script gọi `UrlFetchApp.fetch` thẳng tới
  `{MAIN_SERVER_URL}/queue/submit` (kèm `X-GSBTN-Password` từ `MAIN_SERVER_PASSWORD` nếu có).
  Nếu máy chủ chính phản hồi (kể cả phản hồi lỗi thật — sai mật khẩu, dữ liệu không hợp lệ) thì
  trả thẳng kết quả đó cho xã, **không đệm**. Chỉ khi `UrlFetchApp.fetch` ném lỗi (không tới
  được máy chủ chính) hoặc chưa cấu hình `MAIN_SERVER_URL` thì mới rơi xuống bước đệm dưới đây.
- **Đệm tạm khi không chuyển tiếp được**: Google Sheet `HangDoiPhu` đóng vai trò bảng hàng đợi
  tạm, cấu trúc cột tương ứng `import_queue`; file Excel gốc lưu trong thư mục Google Drive
  theo cấu trúc `HangDoiPhu/<xa>/<tuan>/<ten_file>.xlsx`; script ghi 1 dòng vào Sheet + lưu file
  vào Drive, trả về mã xác nhận.
- **Đồng bộ bù** (`secondary_sync.pull_secondary_queue`, có thể gọi thủ công hoặc theo lịch):
  gọi `POST {action:"list_pending", key:...}` lấy các dòng `status = 'cho_dong_bo'`, tải nội
  dung base64, tạo bản ghi tương ứng trong `import_queue` với `source = 'server_phu'`, rồi gọi
  `POST {action:"mark_synced"}` đánh dấu dòng trên Sheet là `da_dong_bo` (idempotent — không
  kéo trùng nếu chạy nhiều lần).
- Xác thực: Web App yêu cầu khoá `SHARED_KEY` riêng (không dùng chung mật khẩu máy chủ chính)
  cho mọi hành động — mọi xã dùng chung một khoá, chưa có tài khoản riêng từng xã trên GAS (có
  chủ đích, xem `google_apps_script/README.md` mục "Giới hạn đã biết"). Chặng chuyển tiếp
  GAS → máy chủ chính dùng riêng `MAIN_SERVER_PASSWORD`, độc lập với `SHARED_KEY`.
...
- `/queue/submit` chấp nhận **song song hai đường xác thực**, không loại trừ nhau (xem
  `lan_server._handle_queue_submit`): token đăng nhập xã (`commune_token`, dùng bởi `/xa`) HOẶC
  mật khẩu máy chủ dùng chung (`X-GSBTN-Password`, dùng bởi Máy trạm nội bộ và bởi Google Apps
  Script khi chuyển tiếp trực tiếp — GAS chỉ biết một mật khẩu dùng chung, không có token riêng
  từng xã, xem mục 7.2). Có tài khoản xã hay không **không** ảnh hưởng đường mật khẩu dùng
  chung — trước đây thiết kế "hễ có tài khoản là bắt buộc token" đã bị bỏ vì sẽ chặn nhầm chặng
  chuyển tiếp từ GAS.
